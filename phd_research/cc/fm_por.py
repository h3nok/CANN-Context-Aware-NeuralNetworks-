...
class FM_POR(object):
    ...
    def __init__(self, name, input_feature_map, input_size, patch_size, measure=Measure.MI, order=Ordering.Ascending):
...
def test():
    ...
    formatted = tf.convert_to_tensor(
        (slice56 * 255 / np.max(slice56)).astype('uint8'))
