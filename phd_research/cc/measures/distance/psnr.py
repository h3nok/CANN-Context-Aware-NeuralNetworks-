...
def PSNR(patch_1, patch_2, maximumDataValue=255, ignore=None):
    """
    title::
       psnr
    ...
    if type(patch_1).__module__ != numpy.__name__:
        d1 = numpy.asarray(patch_1).flatten()
    else:
        d1 = patch_1.flatten()

    if type(patch_2).__module__ != numpy.__name__:
        d2 = numpy.asarray(patch_2).flatten()
    else:
        d2 = patch_2.flatten()
    ...
    if ignore is not None:
        ...
        error = d1[index].astype(numpy.float64) - \
            d2[index].astype(numpy.float64)
    else:
